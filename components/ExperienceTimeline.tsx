...
export const ExperienceTimeline = () => {
    return (
        <section id="experience" className="py-24 px-6 max-w-7xl mx-auto">
            ...
            <div className="relative border-l border-white/10 ml-4 md:ml-auto md:mr-auto md:border-l-0 md:after:content-[''] md:after:absolute md:after:left-1/2 md:after:top-0 md:after:bottom-0 md:after:w-px md:after:bg-white/10">
                {experiences.map((exp, index) => (
                    ...
                    >
                        {/* Timeline Dot */}
                        <div className={`absolute top-0 w-4 h-4 rounded-full bg-indigo-600 shadow-[0_0_15px_rgba(79,70,229,0.5)] z-10 ${index % 2 === 0
                                ? "left-[-25px] md:left-[-8px]"
                                : "left-[-25px] md:left-auto md:right-[-8px]"
                            }`} />

                        <div className="glass-card p-8 hover:border-indigo-500/30 transition-colors">
                            ...
                            <div className={`flex flex-wrap gap-2 ${index % 2 !== 0 && "md:justify-end"}`}>
                                {exp.skills.map((skill) => (
                                    <span key={skill} className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-xs font-medium text-white/50">
                                        {skill}
                                    </span>
                                ))}
                            </div>
